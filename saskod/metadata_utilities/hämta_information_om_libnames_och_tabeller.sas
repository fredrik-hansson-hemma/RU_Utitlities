...
%get_metadata_library_tables(  )



proc sort data = work.metadata_tables out=work.test;
	by tableuri;
run;

* Kontrollerar om samma fysiska tabell är kopplad till flera metadataobjekt	;
data work.test;
	set work.test;
	by tableuri;
	if not (first.tableuri and last.tableuri);
run;